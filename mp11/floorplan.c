...
int is_leaf_node(node_t* ptr) {
  ...
  /*no need to check for if the pointer is NULL because we will be returning 0 
  anyways if the values on the left and right pointers is NOT a leaf node
  */
  if(ptr -> left == NULL && ptr -> right == NULL){ //return 1 if the values on both pointers is a leaf node
    return 1;
  }
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  ...
  /*since the leaf node and internal node are mutually exclusive, 
  we can return 1 for the internal node when the leaf node is 0
  */
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  if(a == NULL){ //checking for the base case 
     return 0;
  }
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if(ptr -> cutline == 0){
    ptr -> cutline = H; //if the cutline is vertical, change it to horizontal
  }
  else if (ptr -> cutline == 1) {
    ptr -> cutline = V; //if the cutline is horizontal change it to vertical
  }
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  node_t* temp = (node_t*) malloc(sizeof(node_t)); //temporary node to hold 'a'
  *temp = *a; //swap 'a' and 'b'
  *a = *b;
  *b = *temp;
  return; 
...
void swap_topology(node_t* a, node_t* b) {
  ...
  temp_node = a -> parent; //swap parent first
  a -> parent = b -> parent;
  b -> parent = temp_node;

  if(a -> parent -> left == b){ //check which branch a and b were at
    a -> parent -> left = a;
  }
  else{
    a -> parent -> right = a;
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  //base case
  if(n == num_modules - 1){
    ptr -> module = modules + n;  //assign the data to the module pointed by ptr
  ...
  ptr -> right -> module = modules + n; //assign the data to the module pointed by ptr->right
